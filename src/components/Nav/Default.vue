<template>
  <div class="h-auto">
    ...
    >
      ...
      <Color class="ml-2" />
      <div class="md:hidden cursor-pointer ml-2" @click="menuO">Menu</div>
    </div>
    <div
      class="
        md:hidden
    ...
    >
      <nuxt-link
        ...
        :key="`link-${index}`"
        :to="link.link"
        class="bg-gray-300 mx-2 my-1 p-2 rounded-xl dark:(bg-gray-800) transition ease-out duration-700 "
        >{{ link.title }}</nuxt-link
      >
      <div class="h-1 m-2 bg-gray-400"></div>
      <nuxt-link
        to="/signup"
        class="
              bg-gray-300
